...
export default function SearchDialog({ locale }: { locale?: string }) {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className="p-2 hover:bg-white/10 rounded-full transition-colors" aria-label="Search">
        <Search size={20} />
      </button>
              ...
              <div className="max-h-[50vh] overflow-y-auto">
                ...
                {results.map(r => (
                  <Link
                    ...
                    onClick={close}
                    className="flex items-center gap-4 px-5 py-4 hover:bg-[#f8f9fb] transition-colors group"
                  >
                ...
                {results.length === 0 && !query.trim() && (
                  <div className="px-5 py-8 text-center text-[1.3rem] text-gray-400">
                    Type to search across {9} product categories
                  </div>
                )}
              </div>

              <div className="border-t border-gray-200 px-5 py-3 text-[1.1rem] text-gray-400 text-center">
                Press <kbd className="mx-1 rounded bg-gray-100 px-2 py-0.5 text-[1rem] font-mono">Esc</kbd> to close ·{' '}
                <kbd className="mx-1 rounded bg-gray-100 px-2 py-0.5 text-[1rem] font-mono">↑↓</kbd> navigate
              </div>
            </motion.div>
          </motion.div>
        )}
